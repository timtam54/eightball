    ...
    (dx: number, dy: number) => {
      ...
      if (!checkCollision(currentPiece, board, dx, dy)) {
        ...
        if (currentSkin === "blockblast" && linesCleared > 0) {
          ...
          const newColorScheme = BLOCK_BLAST_COLOR_SCHEMES[(blockBlastColorIndex + 1) % BLOCK_BLAST_COLOR_SCHEMES.length]
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 flex flex-col sm:flex-row gap-2 sm:gap-4 w-full sm:w-auto max-w-md">
        <Link href="/" className={skin.buttonStyle} style={{ fontFamily: skin.fontFamily }}>
          ← BACK TO GAMES
        </Link>
        <select
          value={currentSkin}
